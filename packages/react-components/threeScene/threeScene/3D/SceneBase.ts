...
class SceneBase {
   ...
   */
  public async init(domContainer: HTMLElement, assetsData: IFile[]): Promise<void> {
    ...
    // Load assets
    try {
      if (assetsData && assetsData.length > 0) await AssetManager.load(assetsData)
    } catch (error) {
    ...
    const resizeObserver = new ResizeObserver((entries) => this._resize(entries[0]))
  ...
  protected _resize = (containerElement): void => {
    this._rendererSize = {
      width: containerElement?.contentRect?.width || containerElement.clientWidth,
      height: containerElement?.contentRect?.height || containerElement.clientHeight,
    }
